...
public class Crystal_Skill : Skill
{
    ...
    public override void UseSkill()
    {
        ...
        {
            currentCrystal = Instantiate(crystalPrefab,Player.Instance.transform.position, Quaternion.identity);
            Crystal_Skill_Controller currentCrystalScript = currentCrystal.GetComponent<Crystal_Skill_Controller>();

            currentCrystalScript.SetupCrystal(crystalDuration, canExplode, canMoveToEnemy, moveSpeed, FindClosetEnemy(currentCrystal.transform));
        }
        else 
        {
        ...
        if(canUseMultiStacks)
        {
            if(crystalList.Count > 0)
            {
                ...
                newCrystal.GetComponent<Crystal_Skill_Controller>()
                    .SetupCrystal(crystalDuration, canExplode, canMoveToEnemy, moveSpeed, FindClosetEnemy(newCrystal.transform));
